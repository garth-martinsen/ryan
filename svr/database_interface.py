...
arg msg is a dict loaded by ADC  with raw data and augmented by Server with  computations.
 json is used to convert from string dict. Returns bms_id '''
    ...
    def get_lut(self, chan):
        ...
        cu = self.get_cursor()
        select_str = f"select id,vm,vin from luts where CHAN = {chan} and APP_ID ={self.app_id}\
    ...
    def get_ah_total(self):
        '''Returns the most recent ah_total from amp_hrs db table. function 60.'''
    ...
    def get_last_amp_hrs(self):
        '''Selects the last record saved in AMP_HRS table.Used for testing'''
        version= self.version
        app_id=self.app_id
