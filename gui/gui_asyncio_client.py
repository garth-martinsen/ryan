...
#global vars
funct_desc = OrderedDict({
             300: 'save_config(  msg : Config )',       302: 'sync_time()',
             310: 'get_app_config()',                   312: 'get_chan_config( chan : int )',
             ...
             370:'update_lut_pair(_id:int,vm:float,vin:float)' , 380: 'update_lut_timestamp( chan:int )', 
...
async def receiver(reader, handler_dict: OrderedDict):
    ...
    while True:
        ...
        code = data["CODE"]
        # pass the data to the correct handler, which will set state on the App causing an update.
        funct = handler_dict.get( data["CODE"], error_function)
        funct(data)
