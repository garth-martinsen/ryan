# file: /adc/ah_meter.py   purpose: accumulate charge  measured via module ammeter and when reporting,put i_mean, ah_used,in the report...
...
class AhMeter:
    ...
    def accumulate(self):
        ...
        # i=Vs/RS then scale to get true current
        apparent_amps = (vs/self.rs)                
        cs_amps = apparent_amps / self.amp_gain     
        self.add_amps(cs_amps)

    def add_amps(self, amps: float):
        # accumulate and ncrement count so later avg can be returned.
        self.amp_sum += amps
        self.count += 1
    ...
    def report_to_svr(self, report_msg):
        ...
        #reporting time is end_time for the report. ie: Now()
