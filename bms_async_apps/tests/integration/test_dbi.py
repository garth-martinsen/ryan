...
from svr.database_interface_config import APP_CONFIG, CHAN_CONFIG, LUT, BMS, BMS_FIELDS, APP_CONFIG_FIELDS, CHAN_CONFIG_FIELDS, AMP_HRS_FIELDS
...
@pytest.fixture
def amp_hrs():
    # ID │ APP_ID │ VERSION │ MEAS_ID │TIMESTAMP │ I_MEAN │ PERIOD_SEC │ AH_USED │ AH_TOTAL │
    return  deepcopy({"CHAN":3, "MEAS_ID": 99999, "TIMESTAMP": 1780000.01, "I_MEAN": 0.550, "PERIOD_SEC": 3600, "AH_USED":0.550} )
...
#TODO: Figure out how to modify the fixture so fields can be tested...
        ...
        msg["msg_id"]=1234
        msg["chan"]=0
        cols, vals = dbi._create_cols_vals(msg, BMS_FIELDS)
...
#        print(f"setup_data: {setup_data}")
    ...
    #TODO: Fix broken test. Insert is not working so tests fail.
    def test_save_amp_hrs(self, amp_hrs):
        '''Saves to db table AMP_HRS after computing new ah_total '''
        print("input dict: ", amp_hrs )
        prev_ah_total = float(dbi.get_ah_total()[0])
        amp_hrs["AH_TOTAL"]= float(amp_hrs["AH_USED"]) + prev_ah_total
        print("updated amp_hrs: " , amp_hrs)
        dbi.save_amp_hrs( amp_hrs) 
       
        AMP_HRS_ROW = namedtuple("AMP_HRS_ROW", AMP_HRS_FIELDS)
        fetched_saved = AMP_HRS_ROW(*dbi.get_last_amp_hrs())
        print("fetched_saved: ", fetched_saved)
        assert fetched_saved.MEAS_ID == amp_hrs["MEAS_ID"] 
        assert fetched_saved.TIMESTAMP == amp_hrs["TIMESTAMP"] 
        assert fetched_saved.I_MEAN == amp_hrs["I_MEAN"] 
        assert fetched_saved.PERIOD_SEC == amp_hrs["PERIOD_SEC"] 
        assert fetched_saved.AH_USED == amp_hrs["AH_USED"] 
        assert fetched_saved.AH_TOTAL == amp_hrs["AH_USED"]+ prev_ah_total 
        print("PASSED: test_save_amp_hrs(...). Deleting test record from AMP_HRS table.")
        print("PASSED: test_save_amp_hrs(...)")
        dbi.delete_test_amp_hrs()  # removes any records from AMP_HRS with MEAS_ID=9999
